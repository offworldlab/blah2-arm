...
// Global capture pointer for signal handling (must be raw for C signal API).
Capture *CAPTURE_POINTER = nullptr;
...
void record_timing(std::vector<std::string>& names,
  std::vector<double>& times, std::vector<uint64_t>& timestamps,
...
int main(int argc, char **argv)
{
  ...
  std::string file = parse_command_line(argc, argv);
  ...
  // config handling
  std::string contents = read_file_contents(file.c_str());
  ...
  auto capture = std::make_unique<Capture>(type, fs, fc, path);
  CAPTURE_POINTER = capture.get();
  if (state)
  {
  ...
  auto buffer1 = std::make_unique<IqData>(static_cast<uint32_t>(tCpi * tBuffer * fs));
  auto buffer2 = std::make_unique<IqData>(static_cast<uint32_t>(tCpi * tBuffer * fs));

  // run capture
  std::thread t1([&]{
    capture->process(buffer1.get(), buffer2.get(),
      tree["capture"]["device"], ip_capture, port_capture, verbose);
  });

  // setup process CPI
  uint32_t nSamples = fs * tCpi;
  auto x = std::make_unique<IqData>(nSamples);
  auto y = std::make_unique<IqData>(nSamples);
  ...
  {
    std::cerr << "Error in FFTW multithreading." << std::endl;
    return -1;
  }
  ...
  auto ambiguity = std::make_unique<Ambiguity>(
    delayMin, delayMax, dopplerMin, dopplerMax, fs, nSamples, true);
  ...
  auto filter = std::make_unique<WienerHopf>(delayMinClutter, delayMaxClutter, nSamples);
  ...
  auto cfarDetector1D = std::make_unique<CfarDetector1D>(pfa, nGuard, nTrain, minDelay, minDoppler);
  auto interpolate = std::make_unique<Interpolate>(true, true);

  // setup process centroid
  uint16_t nCentroid;
  tree["process"]["detection"]["nCentroid"] >> nCentroid;
  auto centroid = std::make_unique<Centroid>(nCentroid, nCentroid, 1 / tCpi);
  ...
  rangeRes = static_cast<double>(Constants::c) / fs;
  lambda = static_cast<double>(Constants::c) / fc;
  auto tracker = std::make_unique<Tracker>(
    m, n, nDelete, ambiguity->get_cpi(), maxAcc, rangeRes, lambda);

  // setup process spectrum analyser
  double spectrumBandwidth = 2000;
  auto spectrumAnalyser = std::make_unique<SpectrumAnalyser>(nSamples, spectrumBandwidth);
  ...
  auto timing = std::make_unique<Timing>(tStart);
  ...
  std::thread t2([&]{
    while (true)
    {
      ...
      if ((buffer1->get_length() > nSamples) && (buffer2->get_length() > nSamples))
      {
        ...
        buffer1->unlock();
        buffer2->unlock();
        record_timing(timing_name, timing_time, time, "extract_buffer");

        // spectrum
        spectrumAnalyser->process(x.get());
        record_timing(timing_name, timing_time, time, "spectrum");

        // clutter filter
        if (isClutter)
        {
          if (!filter->process(x.get(), y.get()))
          {
            continue;
          }
          record_timing(timing_name, timing_time, time, "clutter_filter");
        }

        // ambiguity process
        map = ambiguity->process(x.get(), y.get());
        map->set_metrics();
        record_timing(timing_name, timing_time, time, "ambiguity_processing");
        ...
        if (isDetection)
        {
          ...
          record_timing(timing_name, timing_time, time, "detector");
        }
        ...
        if (isTracker)
        {
          ...
          record_timing(timing_name, timing_time, time, "tracker");
        }
        ...
        record_timing(timing_name, timing_time, time, "output_radar_data");

        // cpi timer
        time.push_back(current_time_us());
        double delta_ms = static_cast<double>(time.back() - time[0]) / 1000;
...
std::string parse_command_line(int argc, char **argv)
{
  ...
  while (true)
  {
    ...
    case 'h':
      std::cout << "--config <file.yml>: \tSet config file\n"
                   "--help:              \tShow help\n";
      exit(0);
...
void record_timing(std::vector<std::string>& names,
  ...
  const std::string& name)
{
  timestamps.push_back(current_time_us());
  double delta_ms = static_cast<double>(timestamps.back() - timestamps[timestamps.size() - 2]) / 1000;
